...
RED/GREEN tests for the framework-independent 3D Workspace Scene contract.
...
def test_scene_name_is_available() -> None:
    assert (
        _scene(name="Equipment Layout").name
        == "Equipment Layout"
    )
...
def test_scene_rejects_invalid_identity(
    scene_id: object,
) -> None:
    from atlas.application.scene import AtlasScene
...
def test_scene_rejects_invalid_name(
    name: object,
) -> None:
    from atlas.application.scene import AtlasScene
...
def test_scene_node_references_canonical_resource_identity() -> None:
    ...
    assert (
        _node(resource_id=resource_id).resource_id
        == resource_id
    )
...
def test_scene_node_rejects_invalid_node_identity(
    node_id: object,
) -> None:
    from atlas.application.scene import AtlasSceneNode
    from atlas.core.aid import AtlasID
...
def test_scene_node_requires_atlas_id_reference(
    resource_id: object,
) -> None:
    from atlas.application.scene import AtlasSceneNode
...
def test_scene_allows_multiple_nodes_for_resource() -> None:
    """A Resource may have multiple spatial presentations."""
    from atlas.core.aid import AtlasID

    resource_id = AtlasID.generate()
    first = _node("first", resource_id)
    second = _node("second", resource_id)
    scene = _scene()
...
def test_scene_node_rejects_invalid_transform_vectors(
    transform: object,
) -> None:
    with pytest.raises((TypeError, ValueError)):
        _node().set_position(
...
def test_workspace_rejects_invalid_scene() -> None:
    ...
    with pytest.raises(TypeError):
        AtlasWorkspace().set_scene(
            "not a scene",  # type: ignore[arg-type]
        )
